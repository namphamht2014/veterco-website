<div class="container">
  <div class="page-header">
    ...
    <h1><?= $this->lang->line('product_edit')?></h1>
  </div>
  <div class="well">
    <?= validation_errors()?>
    <form class="" action="<?= site_url('Products/sua/'.$sanpham[0]->id)?>" method="post">
      <div class="form-group">
        <label for="ten"><?= $this->lang->line('field_name')?> <span class="text-danger">*</span></label>
        <input type="text" name="ten" class="form-control" value="<?= $sanpham[0]->ten?>">
      </div>
        ...
        <div class="col-sm-4">
          ...
          <select class="form-control" name="dong">
            <?php foreach ($dong as $key => $value): ?>
              <?php if ($value->id == $sanpham[0]->dongID): ?>
        ...
        <div class="col-sm-4">
          ...
          <select class="form-control" name="nhom">
            <?php foreach ($nhom as $key => $value): ?>
              <?php if ($value->id == $sanpham[0]->nhomID): ?>
              <option value="<?= $value->id?>" selected=""><?= $value->ten?></option>
              <?php else: ?>
              <option value="<?= $value->id?>"><?= $value->ten?></option>
              <?php endif; ?>
        ...
        <div class="col-sm-4">
          ...
          <select class="form-control" name="phanloai">
            <?php foreach ($phanloai as $key => $value): ?>
              <?php if ($value->id == $sanpham[0]->phanloaiID): ?>
              <option value="<?= $value->id?>" selected=""><?= $value->ten?></option>
              <?php else: ?>
              <option value="<?= $value->id?>"><?= $value->ten?></option>
              <?php endif; ?>
      ...
      <div class="row form-group">
        <div class="col-sm-3">
          ...
          <textarea name="mota" rows="3" class="form-control"><?= $sanpham[0]->mota?></textarea>
        </div>
        <div class="col-sm-3">
          <label for="chidinh"><?= $this->lang->line('field_indication')?> <span class="text-danger">*</span></label>
          <textarea name="chidinh" rows="3" class="form-control"><?= $sanpham[0]->chidinh?></textarea>
        </div>
        <div class="col-sm-3">
          <label for="chongchidinh"><?= $this->lang->line('field_contra_indication')?></label>
          <textarea name="chongchidinh" rows="3" class="form-control"><?= $sanpham[0]->chongchidinh?></textarea>
        </div>
        <div class="col-sm-3">
          <label for="lieudung"><?= $this->lang->line('field_dosage')?> <span class="text-danger">*</span></label>
          <textarea name="lieudung" rows="3" class="form-control"><?= $sanpham[0]->lieudung?></textarea>
        </div>
      </div>
      <div class="row form-group">
        <div class="col-sm-3">
          <label for="cachdung"><?= $this->lang->line('field_withdraw')?></label>
          <input type="text" name="cachdung" class="form-control" value="<?= $sanpham[0]->cachdung?>">
        </div>
        <div class="col-sm-3">
          ...
          <select class="form-control nhomthu" name="nhomthu" onchange="main.updateLoaithu()">
            <?php foreach ($nhomthu as $key => $value): ?>
              <?php if ($value->id == $sanpham[0]->nhomthuID): ?>
              <option value="<?= $value->id?>" selected=""><?= $value->ten?></option>
              <?php else: ?>
              <option value="<?= $value->id?>"><?= $value->ten?></option>
              <?php endif; ?>
        ...
        <div class="col-sm-3">
          ...
          <div class="col-sm-3">
            ...
            <input type="text" name="khoiluong" class="form-control" value="<?= $sanpham[0]->khoiluong?>">
          </div>
      </div>
      <div class="form-group">
