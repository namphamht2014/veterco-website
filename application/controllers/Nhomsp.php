...
class Nhomsp extends CI_Controller{
  ...
  public function __construct()
  {
    ...
    $this->load->model('Nhomsp_model', 'database');
    $this->lang->load('backend_lang');
  }

  function index()
  {
    $data['title'] = '- '.$this->lang->line('plist_list');
    $data['headers'] = array(
    ...
    $this->load->view('products/nhomsp');
  ...
  function tao()
  {
    ...
    if ($this->form_validation->run() === FALSE) {
      $data['title'] = '- '.$this->lang->line('plist_add');
      $data['headers'] = array(
        base_url().'assets/css/sticky-footer-navbar.css'
      );
      ...
      $this->load->view('template/header', $data);
      $this->load->view('products/taonhomsp');
      $this->load->view('template/footer');
    } else {
      $this->database->save(array('ten' => $this->input->post('ten')));

      redirect('Nhomsp', 'refresh');
    }
  }
  ...
  function sua($id='')
  {
    ...
    if ($this->form_validation->run() === FALSE) {
      $data['title'] = '- '.$this->lang->line('plist_edit');
      $data['headers'] = array(
        base_url().'assets/css/sticky-footer-navbar.css'
      );
      $data['script'] = 'main.activeMenu("products");';
      ...
      $this->load->view('template/header', $data);
      $this->load->view('products/suaNhomsp');
      $this->load->view('template/footer');
    } else {
      $data = array(
        'id' => $id,
