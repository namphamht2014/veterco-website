...
class Dongsp extends CI_Controller{
  ...
  public function __construct()
  {
    ...
    $this->load->model('Dongsp_model', 'database');
    $this->lang->load('backend_lang');
  }

  function index()
  {
    $data['title'] = '- '.$this->lang->line('pline_list');
    $data['headers'] = array(
    ...
    $this->load->view('products/dongsp');
  ...
  function tao()
  {
    ...
    if ($this->form_validation->run() === FALSE) {
      $data['title'] = '- '.$this->lang->line('pline_add');
      $data['headers'] = array(
        base_url().'assets/css/sticky-footer-navbar.css'
      );
      ...
      $this->load->view('template/header', $data);
      $this->load->view('products/taodongsp');
      $this->load->view('template/footer');
    } else {
      $this->database->save(array('ten' => $this->input->post('ten')));

      redirect('Dongsp', 'refresh');
    }
  }
  ...
  function sua($id='')
  {
    ...
    if ($this->form_validation->run() === FALSE) {
      $data['title'] = '- '.$this->lang->line('pline_edit');
      $data['headers'] = array(
        base_url().'assets/css/sticky-footer-navbar.css'
      );
      $data['script'] = 'main.activeMenu("products");';
      $data['dong'] = $this->database->getByID($id);

      $this->load->view('template/header', $data);
      $this->load->view('products/suadongsp');
      $this->load->view('template/footer');
    } else {
      $data = array(
        'id' => $id,
